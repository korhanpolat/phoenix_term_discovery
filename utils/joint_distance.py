...
def dynamic_features(feat_arr, d = 1):
    result = np.zeros_like(feat_arr)
    result[:-d] = feat_arr[d:] - feat_arr[:-d]
    result[-d:] = result[-d-1]
    
    return result


def get_joint_line_static_dynamic(feats_root, seq_name):
    feats_body, shoulder_L = get_body(feats_root, seq_name)
    feats_hand_R = get_hand(feats_root, 'right', seq_name, normalizng_scale=shoulder_L)

    static_body = static_features(feats_body)
    dynamic_body = dynamic_features(static_body)

    static_hand_R = static_features(feats_hand_R)
    dynamic_hand_R = dynamic_features(static_hand_R)

    return np.hstack((static_body, dynamic_body, static_hand_R, dynamic_hand_R))
